...
@section('body')
    <div class="container">
        <div class="row header-row mb-2">
            <div class="col-6 px-0">
                <h1 class="header-text">DATABÁZA</h1>
            </div>
            @if ($errors->any())
    ...
    <div class="container">
        <div class="row content">
            <div class="col-12 px-4 py-4 d-flex">
                <svg class="mt-2" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <select id="action" class="w-100 form-control">
                    <option value="">Vyberte akciu</option>
                    <optgroup label="Texty">
                        <option value="create_text">Vytvoriť text</option>
                    </optgroup>
                    <optgroup label="Fotografie">
                        <option value="create_photo">Vytvoriť fotografiu</option>
                    </optgroup>
                </select>
            </div>
        </div>
    ...
    <div class="container mt-3" id="content-container">
        <div class="row" id="text_model">
            @include('backend.database.texts.Modules.create')
            @include('backend.database.texts.Modules.edit')
        </div>
        <div class="row" id="create_photo_model" style="display: none;">
            @include('backend.database.photos.Modules.create')
            @include('backend.database.photos.Modules.edit')
        </div>
    </div>
...
@section('script')
    <script>
        $(document).ready(function () {
            $("#action").change(function () {
                ...
                $("#text_model, #create_photo_model").hide();

                if (selectedValue === "create_text") {
                    $("#text_model").show();
                    contentContainer.show();
                } else if (selectedValue === "create_photo") {
                    $("#create_photo_model").show();
                    contentContainer.show();
                } else {
                    $("#create_text_model").show();
                }
            });
        });
    ...
    <script>
        function toggleFields(checkbox, textInput, numberInput) {
            $(textInput).prop("disabled", !$(checkbox).is(":checked"));
            $(numberInput).prop("disabled", !$(checkbox).is(":checked"));
        }

        toggleFields('input[name="text2_active"]', 'input[name="text2"]', 'input[name="text2_rows"]');
        toggleFields('input[name="text3_active"]', 'input[name="text3"]', 'input[name="text3_rows"]');

        $('input[name="text2_active"]').change(function () {
            toggleFields(this, 'input[name="text2"]', 'input[name="text2_rows"]');
        });

        $('input[name="text3_active"]').change(function () {
            toggleFields(this, 'input[name="text3"]', 'input[name="text3_rows"]');
        });
    </script>
@endsection
...
{{--    <option value="create_slider">Vytvoriť slider</option>--}}
{{--    <option value="edit_slider">Upraviť slider</option>--}}
{{--</optgroup>--}}
